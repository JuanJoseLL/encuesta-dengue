...
interface ThirdIterationSession {
  id: string;
...
export default function ThirdIterationSessionsPage() {
  const [sessions, setSessions] = useState<ThirdIterationSession[]>([]);
  ...
  useEffect(() => {
    async function loadSessions() {
      try {
        const response = await fetch(`/api/admin/third-iteration/sessions?surveyId=${SURVEY_ID}`);
        ...
        console.error("Error loading third iteration sessions:", err);
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 px-6 py-12">
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <header>
          ...
          <h1 className="mt-4 text-3xl font-bold text-slate-900">Sesiones de Tercera Iteración</h1>
          <p className="mt-2 text-slate-600">
            Supervisión del avance del consenso final en tiempo real
          </p>
        </header>
        ...
        <section className="overflow-hidden rounded-2xl border border-teal-200 bg-white shadow-sm">
          {sessions.length === 0 ? (
            <div className="py-12 text-center text-sm text-slate-500">
              No hay sesiones de tercera iteración aún. Los participantes comenzarán a aparecer aquí cuando accedan a la tercera iteración.
            </div>
          ) : (
            <table className="min-w-full divide-y divide-slate-100 text-sm text-slate-700">
              <thead className="bg-teal-50 text-xs uppercase tracking-[0.2em] text-slate-500">
                <tr>
                  ...
                  <tr key={session.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <ProgressBar
                          value={session.totalStrategies > 0 ? session.reviewedStrategies / session.totalStrategies : 0}
                          size="sm"
                        />
