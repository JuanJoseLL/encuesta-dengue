...
export default function SecondIterationSessionsPage() {
  const [sessions, setSessions] = useState<SecondIterationSession[]>([]);
  ...
  useEffect(() => {
    async function loadSessions() {
      try {
        const response = await fetch(`/api/admin/second-iteration/sessions?surveyId=${SURVEY_ID}`);
        ...
        console.error("Error loading second iteration sessions:", err);
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 px-6 py-12">
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <header>
          ...
          <h1 className="mt-4 text-3xl font-bold text-slate-900">Sesiones de Segunda Iteración</h1>
          <p className="mt-2 text-slate-600">
            Supervisión de avance de revisión y consenso en tiempo real
          </p>
        </header>
        ...
        <section className="overflow-hidden rounded-2xl border border-purple-200 bg-white shadow-sm">
          {sessions.length === 0 ? (
            ...
            <table className="min-w-full divide-y divide-slate-100 text-sm text-slate-700">
              <thead className="bg-purple-50 text-xs uppercase tracking-[0.2em] text-slate-500">
                <tr>
                  ...
                  <tr key={session.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <ProgressBar
                          value={session.reviewedStrategies / session.totalStrategies}
                          size="sm"
                        />
