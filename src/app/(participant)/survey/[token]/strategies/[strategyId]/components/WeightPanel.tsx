...
export function WeightPanel({
  ...
  return (
    <div className={`sticky top-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm ${isSkipped ? "opacity-50 pointer-events-none" : ""}`}>
      ...
      {hasInvalidThresholds && (
        <div className="mt-4 rounded-lg border border-amber-300 bg-amber-50 p-3 flex items-start gap-2">
          <svg
            className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5"
            fill="none"
            viewBox="0 0 24 24"
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-amber-800">
              Algunos indicadores no tienen umbral definido
            </p>
            <p className="text-xs text-amber-700 mt-1">
              Por favor, defina un umbral para todos los indicadores seleccionados antes de continuar.
            </p>
          </div>
        </div>
      )}
