...
export default function ThirdIterationStrategiesPage({
  params,
}: {
    ...
    async function loadProgress() {
      try {
        ...
        if (!progressResponse.ok) {
          throw new Error("Failed to load third iteration progress");
        }
        ...
        const totalStrategies = progressData.totalStrategiesInSurvey || progressData.totalStrategies;
        const availableStrategies = progressData.totalStrategies;
        setSkippedCount(totalStrategies - availableStrategies);
      } catch (error) {
      ...
      <div className="mx-auto max-w-5xl space-y-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-slate-900">
                Tercera Iteración - Consenso final de ponderaciones
              </h1>
              <p className="mt-2 text-slate-600">
                Para cada indicador podrá ver las ponderaciones (pesos) que consignaron los demás
                expertos en la iteración anterior. Úselas para reconsiderar el peso que usted asignó,
                recordando que el objetivo es converger hacia un consenso final. En esta iteración solo
                se trabaja la ponderación del peso.
              </p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-slate-900">
                  Estrategias no disponibles en esta iteración
                </h3>
                <p className="mt-2 text-sm text-slate-700 leading-relaxed">
                  Algunas estrategias de esta tercera iteración no aparecen para usted porque no se
                  pronunció sobre {skippedCount > 1 ? "ellas" : "ella"} en las iteraciones anteriores.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-base leading-relaxed text-indigo-50">
                En esta tercera iteración trabajaremos únicamente sobre un <strong className="font-bold text-white">grupo seleccionado de estrategias</strong>.
                Para cada una verá los indicadores y las <strong className="font-bold text-white">ponderaciones (pesos)</strong> asignadas
                por el grupo en la iteración anterior. <strong className="font-bold text-white">Revise el peso que asignó y decida si lo ajusta o lo deja igual</strong> para
                <strong className="font-bold text-white"> avanzar hacia el consenso final</strong>. Los cambios se guardan automáticamente en una
                copia separada de sus respuestas anteriores, por trazabilidad.
              </p>
            </div>
          </div>
        </div>
        ...
        <section className="grid gap-4">
          {strategies.map((strategy) => (
            <Link
              key={strategy.id}
              href={`/3rdIteration/${token}/strategies/${strategy.id}`}
