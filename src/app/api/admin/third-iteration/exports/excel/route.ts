...
/**
 * GET /api/admin/third-iteration/exports/excel?surveyId=...
 * Exporta todas las respuestas de la tercera iteración como Excel (sin umbrales).
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const worksheet = workbook.addWorksheet("Respuestas Tercera Iteración");
    ...
    sessions.forEach((session) => {
      session.thirdIterationResponses.forEach((response) => {
        // La tercera iteración solo incluye un subconjunto de indicadores.
        if (!isThirdIterationIndicator(response.indicatorId)) return;
    ...
    return new NextResponse(nodeBuffer, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="encuesta-dengue-iteracion-3-${surveyId}-${new Date()
          .toISOString()
          .split("T")[0]}.xlsx"`,
    ...
    console.error("Error exporting third iteration Excel:", error);
