...
interface StrategyStepperProps {
  token: string;
  currentStrategyId?: string;
  steps: StrategyStep[];
}

const statusClasses: Record<StrategyStep["status"], string> = {
...
export function StrategyStepper({ token, currentStrategyId, steps }: StrategyStepperProps) {
  return (
    <ol className="flex flex-col gap-2 rounded-2xl border border-slate-200 bg-white p-4 text-xs text-slate-600">
      {steps.map((step, index) => (
        <li key={step.id}>
          <Link
            href={`/survey/${token}/strategies/${step.id}`}
            ...
            } ${step.id === currentStrategyId ? "ring-2 ring-slate-300" : ""}`}
          >
